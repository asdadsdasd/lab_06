...
// ----------------------------- Игровой процесс ----------------------------
    ...
    private void exchangePlayer(){
        ...
        if(newReproductiveLabel != null)
            addGameListener(newReproductiveLabel);
    ...
    private Point findPlace(Cell cell){
        Point p = cell.position();
        if(new Point(p.x + 1, p.y) == null){
    ...
    private void updateLabels(){
        for(AbstractLabel l : field().labels()){
            if(l.getClass() == ReproductiveLabel.class && ((ReproductiveLabel) l).isReady()){
                ReproductiveLabel label = _labelFactory.createReproductiveLabel();
                label.setPlayer(activePlayer());
                //activePlayer().setLabelTo(new Point (1, 1));
                field().cell(new Point (1, 1)).setLabel(label);
